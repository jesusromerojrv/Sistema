      ...
      <v-flex>
        ...
        >
            ...
            <v-toolbar flat>
              ...
              <v-dialog v-model="adModal" max-width="290">
                <v-card>
                  ...
                  <v-card-actions>
                    <v-spacer></v-spacer>
                    <v-btn @click="activarDesactivarCerrar()" color="green darken-1" text>Cancelar</v-btn>
                    <v-btn v-if="adAccion == 1" @click="activar()" color="orange darken-4" text>Activar</v-btn>
                    <v-btn v-if="adAccion == 2" @click="desactivar()" color="orange darken-4" text>Desactivar</v-btn>
                  </v-card-actions>
                </v-card>
              </v-dialog>
          ...
          <template v-slot:[`item.opciones`]="{ item }">
            ...
            <template v-else>
              <v-icon small @click="activarDesactivarMostrar(1, item)"> done </v-icon>
            </template>
          </template>
                    ...
                    <v-container grid-list-md>
                      <v-layout wrap>
                        <v-flex xs12 sm12 md12 lg12 xl12>
                          <template>
                            <v-text-field v-model="texto" @keyup.enter="listarArticulos()" class="text-center" append-icon="search" label="Búsqueda"></v-text-field>
                            <v-data-table
        ...
        <v-container grid-list-sm class="pa-4 white" v-if="verNuevo">
          <v-layout row wrap>
            ...
            <v-flex xs12 sm2 md2 lg2 xl2>
              <v-btn small fab dark color="teal" @click="mostrarModalArticulos()"><v-icon dark>list</v-icon></v-btn>
            </v-flex>
            ...
            <v-flex xs12 sm12 md12 lg12 xl12>
              <template>
                ...
                <template v-slot:[`item.borrar`]="{ item }">
                  <td v-if="item"><v-icon small class="mr-2" @click="eliminarDetalle(detalles, item)">delete</v-icon></td>
                </template>
                ...
                <template v-slot:[`item.subtotal`]="{ item }">
                  <td class="text-right">${{ (item.cantidad * item.precio) - item.descuento }}</td>
                </template>
                <template v-slot:no-data>
            ...
            <v-flex xs12 sm12 md12 xl12>
              ...
              <v-btn color="success" text v-if="verDetalle == 0" @click.native="guardar()">Guardar</v-btn>
            </v-flex>
          </v-layout>
        </v-container>
      </v-flex>
